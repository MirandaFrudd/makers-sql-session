...
class CountryData
  ...
  def self.population_greater_than_100_million
    result = DatabaseConnection.query("SELECT * FROM countries WHERE density > 100;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
  ...
  def self.all_countries_not_in_europe
    result = DatabaseConnection.query("SELECT * FROM countries WHERE continent IN ('Africa', 'South America', 'Asia', 'North America');")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
  ...
  def self.countries_not_in_asia_hide_gni_per_capita_and_population_density
    result = DatabaseConnection.query("SELECT name, continent, density FROM countries WHERE continent IN ('South America', 'North America', 'Europe', 'Africa');")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
end
